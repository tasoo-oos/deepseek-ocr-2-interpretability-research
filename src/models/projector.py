"""
MLP Projector — maps D2E hidden states to language model embedding space.
"""
...
class MlpProjector(nn.Module):
    """
    Flexible MLP projector supporting several architectures.

    The DeepSeek-OCR-2 default is projector_type="linear" with
    input_dim=896 and n_embed=1280.
    """
    ...
    def __init__(self, cfg: Any):
        ...
        if cfg.projector_type == "identity":
            ...
            for _ in range(1, mlp_depth):
                ...
                modules_list.append(nn.Linear(cfg.n_embed // 2, cfg.n_embed // 2))
            modules = nn.Sequential(*modules_list)
            self.high_layers = nn.Sequential(*modules_list)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.cfg.projector_type == 'low_high_hybrid_split_mlp_gelu':
            ...
            x = torch.concat([self.high_up_proj(high_x), self.low_up_proj(low_x)], dim=-1)
        ...
        if self.cfg.projector_type == 'low_high_split_mlp_gelu':
            high_x, low_x = x[0], x[1]
            return torch.concat([self.high_layers(high_x), self.low_layers(low_x)], dim=-1)

        if self.cfg.projector_type in ('downsample_mlp_gelu', 'normlayer_downsample_mlp_gelu'):
            bs, hw, input_dim = x.shape
            h = w = int(hw ** 0.5)
            pad = (self.cfg.downsample_ratio - h % self.cfg.downsample_ratio) % self.cfg.downsample_ratio
